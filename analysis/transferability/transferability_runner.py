...
ICASE_CONFIG = {
    ...
    # Smooth square waves
    10: {
        'name': 'Tanh Smooth Square',
        'short_name': 'tanh',
        'ylim': (-1.3, 1.3),
        'has_negative': True,
    },
    11: {
        'name': 'Erf Smooth Square',
        'short_name': 'erf',
        'ylim': (-1.3, 1.3),
        'has_negative': True,
    },
    12: {
        'name': 'Sigmoid Smooth Square',
        'short_name': 'sigmoid',
        'ylim': (-1.3, 1.3),
        'has_negative': True,
    },
    ...
    13: {
        'name': 'Multi-Gaussian (Two Pulses)',
        'short_name': 'multi_gaussian',
        'ylim': (-0.1, 2.2),
        'has_negative': False,
    },
    14: {
        'name': 'Bump Function (Compact)',
        'short_name': 'bump',
        'ylim': (-0.1, 1.2),
        'has_negative': False,
    },
    ...
    16: {
        'name': 'Mexican Hat (Ricker)',
        'short_name': 'mexican_hat',
        'ylim': (-0.5, 1.2),
        'has_negative': True,
    },
}
...
def get_icase_config(icase):
    ...
    if icase in ICASE_CONFIG:
        ...
        return {
            'name': f'Unknown (icase={icase})',
            'short_name': f'icase{icase}',
            'ylim': (-1.5, 2.5),
            'has_negative': True,
        }
...
def generate_filename(model_path, training_params, plot_mode, extension, icase=None):
    ...
    if icase is not None:
        icase_config = get_icase_config(icase)
        icase_prefix = f"icase{icase}_{icase_config['short_name']}_"
    else:
        icase_prefix = ""
    
    if training_params:
        param_str = (f"gamma_{training_params['gamma_c']}_"
                    f"step_{training_params['step_domain_fraction']}_"
                    f"rl_{training_params['rl_iterations_per_timestep']}_"
                    f"budget_{training_params['element_budget']}")
    else:
        # Fallback to model directory name
        param_str = Path(model_path).parent.name
    
    filename = f"{icase_prefix}{param_str}_{plot_mode}.{extension}"
    return filename
...
def create_animation(times, solutions, grids, coords, solver, training_params, 
    ...
    icase_str = create_icase_title(solver.icase)
...
def create_snapshot(times, solutions, grids, coords, solver, training_params,
    ...
    sim_config_str = create_simulation_config_title(solver, initial_refinement, element_budget)
    icase_config = get_icase_config(solver.icase)
    fig.suptitle(f'Transferability Test: {icase_config["name"]}\n{param_str}\n{sim_config_str}',)
...
def run_single_model(model_path, time_final=1.0, element_budget=50, max_level=5, 
    ...
    results = {
        ...
        'icase': icase,
        'icase_name': get_icase_config(icase)['name'],
        'icase_short_name': get_icase_config(icase)['short_name'],
...
def main():
    ...
    if args.plot_mode:
        if args.output_dir:
            ...
            output_dir = create_output_directory(args.model_path, args.icase)
    else:
        output_dir = None
    ...
    print(f"Test Case: {icase_config['name']} (icase={args.icase})")
